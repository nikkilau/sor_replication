...
\begin{itemize}
  ...
  \item \path{results/extension_calibration_metrics.csv} 与 \path{results/extension_calibration_heatmap.png}：扩展校准实验。
\end{itemize}

根目录 \path{results/} 中也存在历史输出。由于这些文件可能来自修正之前的版本，最终分析应优先采用 \path{results/parameter_variants/} 下的新输出。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{results/extension_calibration_heatmap.png}
\caption{扩展校准综合损失热图。}
\end{figure}
...
\begin{itemize}
  ...
  \item \path{results/extension_calibration_metrics.csv}
\end{itemize}
